...
{ 
    ...
    Param ( 
        ...
        [string]$LogFileName="Active_Employees_from_DarwinBox"
    ) 
...
#region Get Active DarwinBox Employees and Export to CSV

Write-Log "Getting active Employees from DarwinBox ..."

$Active_EmployeesList = Get-AllEmployees -admin_Email $DWB_adminEmail -secretKey $DWB_secretKey `
                                           -UID $DWB_UID -datasetKey $DWB_active_dts_key

if ($Active_EmployeesList.status -eq 1)
{
    $Active_EmployeesList.employee_data | Export-CSV $DWB_ActiveEmployees_output_CSV -NoTypeInformation -Encoding UTF8

    Write-Log "Employees exported successfully to CSV file"
}
else
{
    Write-Log "No users found in the list of Active DarwinBox Employees." -Level Warn
}
